    ...
    <div class="col-2">
      ...
      <form @submit.prevent="register" class="card card-form">
        ...
        <div class="form-group">
          ...
          <input v-model.lazy="form.username" @blur="$v.form.username.$touch()"
                 id="username"
                 type="text"
                 class="form-input">
          <template v-if="$v.form.username.$error">
            <span v-if="!$v.form.username.required"
                  class="form-error">This field is required</span>
            <span v-if="!$v.form.username.unique"
                  class="form-error">This username is taken</span>
          </template>
        </div>

        <div class="form-group">
          <label for="email">Email</label>
          <input v-model.lazy="form.email" @blur="$v.form.email.$touch()"
                 id="email"
                 type="email"
                 class="form-input">
          <template v-if="$v.form.email.$error">
            <span v-if="!$v.form.email.required"
                  class="form-error">This field is required</span>
            <span v-if="!$v.form.email.unique"
                  class="form-error">This email is taken</span>
          </template>
        </div>
...
import { uniqueUsername, uniqueEmail, supportedImageFile, imageOk } from '@/utilities/validators';
